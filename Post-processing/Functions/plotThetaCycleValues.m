function plotThetaCycleValues(dataSets, comment_group1, savenum)
...
%     comments = cell2mat(params.comments);
    ...
    for iID = 1:nIDs
        ...
        if ~isfield(file.(fieldName),'CWTcycleValues') || ~all(file.cycles == cycles)
            file = detectArtifacts(file); % skip theta cycles with spikes or artifacts
            cycles_noArtifacts = reshape(cycles(~file.(fieldName).spikes(cycles)),1,[]); % row vector
            cycle_start_index_noArtifacts = file.cycle_start_index(cycles_noArtifacts); % assumes 1 trial of data
            [~,~,~,file.cell.CWTcycleValues] = thetaCWT(file,fieldName,cycle_start_index_noArtifacts);
