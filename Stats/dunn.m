function Qtable = dunn(x,g,varargin)
...
f=(tot*(tot+1)/12)-(t/(6*(tot-1))); %constant denominator factor with ties correction
...
disp(' ')
clear Idx R r t tot %clear unnecessary variables

count=0;
switch ctrl
    case 0 %Without control group
        kstar = 0.5*k*(k-1); % number of comparisons
...
Qtable = cell2table(pb,'VariableNames',{'Comparison','Q_value','Crit_Q','Comment','P-Value (Sidak)'});
disp(Qtable)
...
function Qvalue
    ...
    if isempty(gnames)
        ...
        pb(count,1:3)={strcat(gnames(Mr(I,2)),'-',gnames(Mr(J,2))) Q vcrit}; %vectors of comparisons
    end
    ...
    pval = 2*(1 - normcdf(abs(Q)) ); % p-value of qvalue or z statistic
    pb(count,5) = 1 - (1-pval).^kstar; % p-value sidak adjustment for multiple comparisons
end
end